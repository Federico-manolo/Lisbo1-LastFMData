...
interface LastFMToArtistDataResolver {
    fun getArtistFromExternalData(artistName: String?): LastFMArtistData?
}

internal class JSONToArtistDataResolver : LastFMToArtistDataResolver {

    override fun getArtistFromExternalData(artistName: String?): LastFMArtistData? =
        try {
            artistName?.let {
                val jObjectArtist = artistResponseToJson(artistName)
                LastFMArtistData(jObjectArtist.getArtistName(), jObjectArtist.getArtistBioContentToString(), jObjectArtist.getArtistURL()) }
        } catch (e: Exception) {
            null
        }
